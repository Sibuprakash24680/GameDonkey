...
parts = [
    ...
    '<g fill="#C8A96B">' + ell(NECK) + ell(EAR_L) + ell(EAR_R) + '</g>',
    '<g fill="#2E382C">' + ell(EAR_L_IN) + ell(EAR_R_IN) + '</g>',
    '<g fill="#C8A96B">' + ell(SKULL) + ell(BRIDGE) + ell(MUZ) + '</g>',
    '<path d="' + poly(MANE) + '" fill="#2E382C"/>',
    '<circle cx="%s" cy="%s" r="1.8" fill="#2E382C"/>' % (EYE[0], EYE[1]),
    '<g clip-path="url(#m)"><circle cx="%s" cy="%s" r="%s" fill="#F7F4ED"/></g>' % (CREAM_CAP[0], CREAM_CAP[1], CREAM_CAP[2]),
    '<circle cx="%s" cy="%s" r="%s" fill="#2E382C"/></svg>' % (NOSTRIL[0], NOSTRIL[1], NOSTRIL[2]),
]
fav = "".join(parts)
