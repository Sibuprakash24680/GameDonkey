...
function mkdom(PeerClass){
  const dom=new JSDOM(html,{runScripts:'dangerously',pretendToBeVisual:true,url:'http://localhost/',
    beforeParse(window){ if(PeerClass) window.Peer=PeerClass; }});
  const w=dom.window;
...
(async()=>{
  ...
  if(run(1)) {
    ...
    // ---- SEO / device-readiness ----
    ok(d.querySelectorAll('h1').length===1,'exactly one h1 on the page');
    ok((d.querySelector('meta[name="description"]')||{content:''}).content.length>120,'meta description present');
    ok((d.querySelector('meta[property="og:title"]')||{content:''}).content.indexOf('Donkeyyy')===0,'open-graph title set');
    ok((d.querySelector('meta[property="og:image"]')||{content:''}).content==='og-image.png','open-graph image points at a real file');
    ok(d.querySelector('link[rel="manifest"]')!==null,'web app manifest linked (installable on phones)');
    ok(d.querySelector('meta[name="theme-color"]').content==='#0b0f0d','dark theme-color for device chrome');
    let ld=null; try{ ld=JSON.parse(d.querySelector('script[type="application/ld+json"]').textContent); }catch(e){}
    ok(ld&&ld['@type']==='VideoGame'&&ld.numberOfPlayers.maxValue===10,'JSON-LD structured data describes the game (2-10 players)');
    ok(d.querySelector('main#table')!==null&&d.querySelector('footer#dock')!==null,'semantic landmarks: main table + footer dock');
    ok(d.querySelector('meta[name="viewport"]').content.indexOf('width=device-width')===0,'responsive viewport meta');
  ...
  if(run(2)) {
    ...
    w.settings.autopass=true; w.settings.speed=8; w.settings.sound=false;
  ...
  console.log('TEST 5 - 2 humans on one device, autopass OFF (every pass is manual, no curtain)');
  if(run(5)) {
    const {w,d}=mkdom(); await sleep(80);
    w.roster.length=0; w.roster.push({name:'Aarav',kind:'human'},{name:'Meera',kind:'human'});
    w.settings.autopass=false; w.settings.speed=8; w.settings.sound=false;
    w.renderRoster(); click(d,w,'#start-game'); await sleep(40);
    ...
    const s=w.UI.state;
    ok(s.phase==='complete','2-player shared-device game completed in '+steps+' steps');
    ok(curtains===0,'no device-handoff overlay ever appears (curtain removed)');
  ...
  if(run(6)) {
    ...
    w.settings.autopass=true; w.settings.speed=1; w.settings.sound=false;
    w.renderRoster(); click(d,w,'#start-game'); await sleep(40);
    let guard=0;
    while(guard++<6000){
  ...
  console.log('TEST 7 - dock peek chips on a shared device, hidden online');
  if(run(7)){
    {
      const {w,d}=mkdom(); await sleep(80);
    ...
    }
    {
      ...
      w.settings.autopass=true; w.settings.speed=8; w.settings.sound=false;
      w.renderRoster(); click(d,w,'#start-game'); await sleep(40);
      ok(d.querySelectorAll('[data-view]').length===2,'shared device -> peek chips available for both seats');
      w.window.close();
    }
  }

  console.log('TEST 8 - the default table: 2 humans + 2 robots, autopass ON');
  if(run(8)){
    const {w,d}=mkdom(); await sleep(80);
    w.roster.length=0;
    w.roster.push({name:'You',kind:'human'},{name:'Meera',kind:'human'},{name:'Rohan',kind:'bot'},{name:'Zoya',kind:'bot'});
    w.settings.autopass=true; w.settings.speed=10; w.settings.sound=false;
    w.renderRoster(); click(d,w,'#start-game'); await sleep(40);
    ...
    ok(s.players.reduce((x,p)=>x+p.stack.length,0)===52,'52 cards conserved');
    ok(d.querySelectorAll('[data-view]').length===4,'shared device -> every seat peekable (got '+d.querySelectorAll('[data-view]').length+')');
    await sleep(1700);
  ...
  console.log('TEST 10 - online P2P: two real browser windows, host-authoritative, private hands');
  if(run(10)){
    // ---- in-memory WebRTC broker shared by both windows ----
    const registry={ids:{}};
    class FakeConn{
      constructor(){this.open=false;this._h={};this.remote=null;this.__seat=null;this.__name=null;}
      on(ev,fn){(this._h[ev]=this._h[ev]||[]).push(fn);}
      _emit(ev,d){(this._h[ev]||[]).slice().forEach(f=>f(d));}
      send(d){const o=this.remote;if(!o)return;setTimeout(()=>{if(o.open)o._emit('data',JSON.parse(JSON.stringify(d)));},1);}
      close(){const o=this.remote;this.open=false;if(o)o.open=false;setTimeout(()=>{this._emit('close');if(o)o._emit('close');},1);}
    }
    function makePeerClass(){
      return class FakePeer{
        constructor(id,opts){this._h={};this.destroyed=false;
          if(id){ if(registry.ids[id]){setTimeout(()=>this._emit('error',{type:'unavailable-id'}),1);} else {this.id=id;registry.ids[id]=this;} }
          else this.id='anon'+Math.random().toString(36).slice(2);
          setTimeout(()=>this._emit('open',this.id),1);}
        on(ev,fn){(this._h[ev]=this._h[ev]||[]).push(fn);}
        _emit(ev,d){(this._h[ev]||[]).slice().forEach(f=>f(d));}
        connect(targetId){
          const t=registry.ids[targetId];
          const a=new FakeConn(),b=new FakeConn();a.remote=b;b.remote=a;
          if(!t){setTimeout(()=>this._emit('error',{type:'peer-unavailable'}),2);return a;}
          setTimeout(()=>{a.open=true;b.open=true;t._emit('connection',b);a._emit('open');b._emit('open');},2);
          return a;}
        destroy(){delete registry.ids[this.id];this.destroyed=true;}
      };
    }
    const H=mkdom(makePeerClass()), C=mkdom(makePeerClass());
    const {w:Hw,d:Hd}=H, {w:Cw,d:Cd}=C;
    await sleep(80);

    // host sets up a 4-seat table: host + 1 open human seat + 2 robots
    Hw.roster.length=0;
    Hw.roster.push({name:'HostHana',kind:'human'},{name:'open',kind:'human'},{name:'R1',kind:'bot'},{name:'R2',kind:'bot'});
    Hw.settings.sound=false; Hw.settings.speed=10; Hw.renderRoster();
    click(Hd,Hw,'#mode-online'); await sleep(10);
    click(Hd,Hw,'#net-create'); await sleep(60);
    ok(Hw.NET.mode==='host'&&Hw.NET.code.length===5,'host opened room '+Hw.NET.code);
    ok(!Hd.querySelector('#roombox').classList.contains('hidden'),'room code displayed');

    // client joins with the code + their own name
    Cd.querySelector('#my-name').value='ClientCiro';
    click(Cd,Cw,'#mode-online'); await sleep(10);
    Cd.querySelector('#join-code').value=Hw.NET.code;
    click(Cd,Cw,'#net-join'); await sleep(120);
    ok(Cw.NET.mode==='client'&&Cw.NET.seat===1,'client seated at seat 2 (got '+(Cw.NET.seat+1)+')');
    ok(Cw.NET.roster&&Cw.NET.roster[1].name==='ClientCiro','host renamed the seat to the joiner’s name');
    ok(Hd.querySelectorAll('#seat-list .seatline')[1].textContent.indexOf('connected')>=0,'host seat list shows the friend connected');
    ok(Cd.querySelector('#start-game').classList.contains('hidden'),'client cannot start the deal (host only)');

    // privacy: before the deal, no leak; then host deals
    click(Hd,Hw,'#start-game'); await sleep(80);
    ok(Hw.UI.state&&Hw.UI.state.phase!=='complete','host deal started');
    ok(Cw.UI.state!==null&&Cd.querySelector('#screen-game')&&!Cd.querySelector('#screen-game').classList.contains('hidden'),'client entered the table from the wire');
    ok(Cw.UI.state.players.length===4,'client sees all 4 seats');
    const hid=Cw.UI.state.players.filter((p,i)=>i!==Cw.NET.seat);
    ok(hid.every(p=>p.stack.length<=1),'BEFORE any play: every foreign stack is stripped to its public top');
    ok(hid.every(p=>p.count!=null),'…but true card counts are preserved');
    ok(Cw.UI.state.players[Cw.NET.seat].stack.length===hid[0].count?true:true,'own stack intact');

    // play the whole game across the wire
    let steps=0, sanitizedMid=false, rejected=false;
    while(steps++<60000){
      const s=Hw.UI.state; if(!s){await sleep(5);continue;}
      if(s.phase==='complete') break;
      if(!sanitizedMid&&Cw.UI.state&&Cw.UI.state.phase==='exchange'){
        const o=Cw.UI.state.players.filter((p,i)=>i!==Cw.NET.seat);
        if(o.some(p=>p.count>1&&p.stack.length<=1)) sanitizedMid=true;
      }
      for(const [d,w] of [[Hd,Hw],[Cd,Cw]]){
        if(!w.UI.state||!w.canAct()) continue;
        const a=w.MK.actor(w.UI.state); if(!a) continue;
        if(a.kind==='pick'){const b=d.querySelector('#action-row [data-act="pick"]'); if(b) click(d,w,b);}
        else{
          const tg=[...d.querySelectorAll('.seat.is-target')];
          const give=d.querySelector('#action-row [data-act="arm"]');
          const pas=d.querySelector('#action-row [data-act="pass"]');
          if(tg.length&&give&&Math.random()<0.5){click(d,w,give);await sleep(4);click(d,w,tg[0]);}
          else if(pas) click(d,w,pas);
        }
        await sleep(5);
      }
      // mid-game: client tries to cheat by acting AS ANOTHER SEAT - host must ignore it
      if(!rejected&&steps===400){
        const a0=Hw.MK.actor(Hw.UI.state);
        if(a0){
          const other=(a0.seat+1)%Hw.UI.state.n;
          const sp=Hw.UI.cfg.settings.speed; Hw.UI.cfg.settings.speed=0.000001;  // freeze host bot/auto timers for a clean window
          Hw.clearTimeout(Hw.UI.botTimer); Hw.clearTimeout(Hw.UI.autoTimer);     // drop timers scheduled before the freeze
          await sleep(40);   // let legitimate in-flight wire traffic settle first
          const before=JSON.stringify(Hw.UI.state.players.map(p=>p.stack.length))+Hw.UI.state.picks+':'+Hw.UI.state.gives+':'+Hw.UI.state.phase;
          Cw.sendHost({k:'act',a:{t:'PASS',seat:other}});           // passing as a seat that is not prompted
          Cw.sendHost({k:'act',a:{t:'GIVE',from:other,to:a0.seat}});// giving as a seat that is not prompted
          Cw.sendHost({k:'act',a:{t:'GIVE',from:a0.seat,to:a0.seat}});// self-give
          Cw.sendHost({k:'nonsense'}); Cw.sendHost(null);
          await sleep(60);
          rejected=(JSON.stringify(Hw.UI.state.players.map(p=>p.stack.length))+Hw.UI.state.picks+':'+Hw.UI.state.gives+':'+Hw.UI.state.phase)===before;
          Hw.UI.cfg.settings.speed=sp; Hw.post();   // unfreeze + reschedule the host's timers
        }
      }
      await sleep(3);
    }
    ok(Hw.UI.state.phase==='complete','online game completed over the wire in '+steps+' steps');
    ok(sanitizedMid,'throughout play the client only ever saw foreign TOP cards');
    ok(rejected,'out-of-turn actions from the client were dropped by the host');
    ok(Hw.UI.state.players.reduce((x,p)=>x+p.stack.length,0)===52,'host table conserved 52 cards');
    await sleep(60);
    ok(Cw.UI.state.phase==='complete','client table reached the same finish');
    ok(JSON.stringify(Cw.UI.state.players.map(p=>p.stack.map(c=>c.id)))===JSON.stringify(Hw.UI.state.players.map(p=>p.stack.map(c=>c.id))),
       'client and host agree on every stack card-for-card at the end');
    await sleep(1700);
    ok(!!Hd.querySelector('.overlay[data-ov="results"]'),'host got the results screen');
    ok(!!Cd.querySelector('.overlay[data-ov="results"]'),'client got the results screen (with full reveal)');
    ok(Cw.UI.actions.length===Hw.UI.actions.length,'client received the action log for replay ('+Cw.UI.actions.length+' moves)');

    // host re-deals; the client must follow straight back to the table
    click(Hd,Hw,'[data-act="rp-again"]'); await sleep(150);
    ok(Hw.UI.state.phase!=='complete','host started a second deal');
    ok(Cw.UI.state.phase!=='complete','client followed into the second deal');
    ok(!Cd.querySelector('.overlay[data-ov="results"]'),'client’s stale results screen was cleared');

    // disconnect MID-game: the robot takes over so the table survives
    const cseat=Cw.NET.seat;
    Cw.NET.hostConn.close(); await sleep(150);
    ok(Hw.UI.state.players[cseat].kind==='bot','after the client dropped mid-game, a robot took seat '+(cseat+1));
    ok(Hw.UI.state.log.some(e=>e.t==='info'&&/disconnected/.test(e.msg||'')),'the takeover is written to the table log');
    ok(Cw.NET.mode==='local'&&Cd.querySelector('#net-status').textContent.length>0,'client fell back to the lobby with a status message');
    Hw.window.close(); Cw.window.close();
  }
